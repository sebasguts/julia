        ...
        if compact.idx-1 == inline_idx
            ...
            if isapply
                ...
                for arg in argexprs[3:end]
                    tupT = compact_exprtype(compact, arg)
                    t = widenconst(tupT)
                    for i = 1:length(t.parameters)
                        # Insert a getfield call here
                        new_call = Expr(:call, Core.getfield, arg, i)
                        new_call.typ = getfield_tfunc(tupT, Const(i))
                        push!(new_argexprs, insert_node_here!(compact, new_call, new_call.typ, compact.result_lines[idx]))
                    end
                end
    ...
    for (idx, stmt) in pairs(ir.stmts)
        ...
        if invoke_data === nothing
            ...
            if meth === false || length(meth) != 1
                (!isapply && !isinvoke) && maybe_make_invoke!(ir, idx, stmt.typ, atypes, sv, atype_unlimited, nothing)
                continue
            end
        ...
        methsig = method.sig
        if !(atype <: metharg)
            (!isapply && !isinvoke) && maybe_make_invoke!(ir, idx, stmt.typ, atypes, sv, atype_unlimited, invoke_data)
            continue
        end
