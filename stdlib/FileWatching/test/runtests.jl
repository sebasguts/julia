        ...
        for j = 1:4
            for i = 3:-1:1
                while changes[end - 1][1] == "$F_PATH$i"
                    @test pop!(changes) == ("$F_PATH$i" => FileWatching.FileEvent(FileWatching.UV_CHANGE))
                end
                @test pop!(changes) == ("$F_PATH$i" => FileWatching.FileEvent(FileWatching.UV_RENAME))
            end
        end
    end
    @test all(x -> (length(x) == 2 && x[1] == F_PATH && (x[2].changed || x[2].renamed) && !x[2].timedout), changes) || changes
end
